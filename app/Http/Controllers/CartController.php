...
use App\Models\CartItemsImport;
...
class CartController extends Controller
{
    ...
    public function cartData( Request $request )
    {
      if ( isset($request['productIds']) && count($request['productIds']) > 0 ) {
        ...
        $productItems = ProductWithStockResource::collection($productItemsCollection);
    ...
    if (!extension_loaded('zip')){
          return response()->json([
            'success' => false,
            'status' => 'error',
            'message' => translate('Please enable the Zip extension.')
          ]);
      }
      ...
      if ($request->hasFile('cart_file')) {
        ...
        $directory = "users/{$user_id}/cart_uploaded_excel_files";
        Storage::disk('public')->makeDirectory($directory);

        $file = $request->file('cart_file');
        $filename = time() . '_' . $file->getClientOriginalName();
        $path = $file->storeAs($directory, $filename, 'public');

        CartUploadedExcelFile::create([
            'user_id' => $user_id,
            'file_path' => $path,
        ]);


          $import = new CartItemsImport;
          Excel::import($import, request()->file('cart_file'));
          $importedData = $import->getItems();
          $rowCount = $import->getRowCount();

          return response()->json([
            'success' => true,
            'status' => 'success',
            'message' => translate('Cart items imported successfully.'),
            'data' => [
              'all_barcodes' => $importedData['all_barcodes'],
              'valid_barcodes_count' => $importedData['count_valid_barcodes'],
              'filtered_quantities' => $importedData['filtered_quantities'],
              'total_rows' => $rowCount
            ]
          ]);
      } else {
