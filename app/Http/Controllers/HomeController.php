...
class HomeController extends Controller
{
    ...
    {
        return (new AboutUsService)->index();
    }
  ...
  public function userDashboard(Request $request){
    ...
    return (new UserDashboardService())->userDashboard($request);

  }

  public function userBasicProfileUpdate(Request $request)
  {
      $user = Auth::user();
      ...
      $data = $request->validate([
          ...
          'phone.regex' => 'The phone number must be in the format +11234567890.',
        ]);
      ...
      return back()->with('title', 'Changes Saved!')
                  ->with('message', 'Your account details have been successfully updated. These details will now be used throughout your shopping journey.');
  }
  ...
  public function userPasswordUpdate(Request $request)
  {
      ...
      return back()->with('title', 'Password Changed!')
                  ->with('message', 'You can now use your new password to login to your account.');
  }



  public function userAccountAddresses(Request $request){
    $user = Auth::user();

    return inertia('UserDashboard/Account/AccountAddresses', [
        'shippingAddress' => $user->addresses()->where('address_type', 'shipping')->first(),
        'billingAddress' => $user->addresses()->where('address_type', 'billing')->first(),
    ]);
  ...
  public function userShippingAddress(Request $request){
    ...
    $states = State::where('country_id', $country->id)
                    ...
                    ->get();
    // Format
    $formattedCities = $states->map(function ($state) {
        return [
            'label' => $state->name,
            'value' => $state->id
        ];
    ...
    if ($shippingAddress) {
        ...
        $data['savedAddress'] = [
            ...
            'address_line_two' => $shippingAddress->address_line_two,
    ...
    return inertia('UserDashboard/Account/Partials/ShippingAddress', $data);
  }
  ...
  public function storeShippingAddress(Request $request)
  {
      ...
      $validated['address_type'] = 'shipping';
      $validated['country_id'] = 231;
      ...
      return back()->with('title', 'Shipping Address Updated!')
                  ->with('message', 'Your shipping address has been successfully updated, this address will now be used for all future orders.');
  }
  ...
  public function userBillingAddress(Request $request){
    ...
    return inertia('UserDashboard/Account/Partials/BillingAddress', $data);
  }
  ...
  public function storeBillingAddress(Request $request)
  {
      ...
      // Set address type and user ID
      $validated['address'] = $validated['address_line_one'] . " " .  $validated['address_line_two'];
      $validated['address_type'] = 'billing';
      $validated['country_id'] = 231;
      $validated['user_id'] = $user->id;
      ...
      return back()->with('title', 'Billing Address Updated!')
                  ->with('message', 'Your billing address has been successfully updated, this address will now be used for all future orders.');
  }


  public function StoreAddress(Request $request){
    return (new UserDashboardService())->StoreAddress($request);
  }

  public function UpdateAddress(Request $request, Address $address){
    return (new UserDashboardService())->UpdateAddress($request, $address);
  }

  public function UpdateAddressType(Request $request, Address $address)
  {
      return (new UserDashboardService())->UpdateAddressType($request, $address);
  }

  public function DeleteAddress(Address $address)
  {
      return (new UserDashboardService())->DeleteAddress($address);
  }
    ...
    public function old_all_brands(Request $request)
    {
        $brands = Brand::all();
