...
if (!function_exists('uploaded_asset')) {
    function uploaded_asset($id , $isAbsolute = true)
    {
        if (($asset = Upload::find($id)) != null) {
            if(!$isAbsolute){ return $asset?->file_name; }
            return !isset($asset?->external_link) || !$asset?->external_link ? ( isset($asset?->file_name) ? my_asset($asset?->file_name) : null) : $asset?->external_link;
        }
        $placeholder = 'assets/img/placeholder.jpg';
        if(!$isAbsolute){ return $placeholder; }
        return static_asset($placeholder);
    }
}
...
if (!function_exists('get_setting')) {
    function get_setting($key = null, $default = null, $lang = false , $onlySelectedLang = false)
    {
        ...
        if ($lang === false) {
            $setting = $settings->where('type', $key)->first() ?? null;
        } else {
            $setting = $settings->where('type', $key)->where('lang', $lang)->first() ?? null;
            $setting = !$setting && !$onlySelectedLang ? $settings->where('type', $key)->first() : $setting;
        }
        $settingVal =  $setting?->value ?? null;
        $finalSettingVal = $settingVal === null ? $default : $settingVal;
        // if ( $key == 'we_help_partners_description' ) dd($finalSettingVal);
        return $finalSettingVal;
    }
}

if (! function_exists('customTrans')) {
    function customTrans($key, $locale = null) {
        $locale = $locale ?: app()->getLocale();

        $file = base_path("customTranslations/{$locale}/custom.php");
        ...
        return $key; // Fallback if file does not exist
    }
}

if (! function_exists('customTransWithReplace')) {
  function customTransWithReplace($key, array $replace = [], $locale = null)
  {
      $locale = $locale ?: app()->getLocale();
      $file = base_path("customTranslations/{$locale}/custom.php");
      ...
      if (file_exists($file)) {
          ...
          // Replace placeholders like :id or :name in the translation string
      ...
      return $key; // fallback if file not found
  }
}
