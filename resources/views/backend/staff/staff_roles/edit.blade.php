      ...
      <div class="card-body p-0">
        ...
        <form class="p-4" action="{{ route('roles.update', $role->id) }}" method="POST">
          ...
          <div class="form-group row">
            <label class="col-md-3 col-from-label" for="name">{{translate('Name')}} <i class="las la-language text-danger"
          ...
          <div class="card-header">
            ...
            <div class="d-flex">
              <button class="btn btn-primary" type="button"
                onclick="document.querySelectorAll('input[type=checkbox]').forEach(e => { e.checked = true })">{{ translate('Select All') }}</button>
              <div class="p-1"> </div>
              <button class="btn btn-secondary" type="button"
                onclick="document.querySelectorAll('input[type=checkbox]').forEach(e => { e.checked = false })">{{ translate('Deselect All') }}</button>
            </div>
          </div>
          <br>
          @php
          ...
          @foreach ($permission_groups as $key => $permission_group)
            ...
            @if($show_permission_group)
              <ul class="list-group mb-4">
                <li class="list-group-item bg-light d-flex justify-content-between" aria-current="true">
                  {{ translate(Str::headline($permission_group[0]['section'])) }}
                  <div class="d-flex">
                    <button class="btn-sm btn-primary" type="button"
                      onclick="document.querySelectorAll(`input[type=checkbox][section={{ $permission_group[0]['section'] }}]`).forEach(e => { e.checked = true })">{{ translate('Select All') }}</button>
                    <span class="p-1"> </span>
                    <button class="btn-sm btn-secondary" type="button"
                      onclick="document.querySelectorAll(`input[type=checkbox][section={{ $permission_group[0]['section'] }}]`).forEach(e => { e.checked = false })">{{ translate('Deselect All') }}</button>
                  </div>
                </li>
                      ...
                      <div class="col-lg-2 col-md-3 col-sm-4 col-xs-6">
                        <div class="p-2 border mt-1 mb-2">
                          ...
                          <label class="aiz-switch aiz-switch-success">
                            <input type="checkbox" name="permissions[]" class="form-control demo-sw" section="{{ $permission->section }}"
                              value="{{ $permission->id }}" @if ($role->hasPermissionTo($permission->name)) checked @endif>
