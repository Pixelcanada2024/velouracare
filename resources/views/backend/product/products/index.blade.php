            ...
            <table class="table aiz-table mb-0">
                <thead>
                    <tr>
                        ...
                        <th data-breakpoints="lg">{{translate('Best Offers')}}</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input onchange="update_new_arrival(this)" value="{{ $product->id }}" type="checkbox" <?php if ($product->new_arrival == 1) echo "checked"; ?> >
                                <span class="slider round"></span>
                            </label>
                        </td>
                        <td>
                        ...
                        <td class="text-right">
                            <a class="btn btn-soft-success btn-icon btn-circle btn-sm"  href="{{ route('react.product', $product->id) }}" target="_blank" title="{{ translate('View') }}">
...
@section('script')
    <script type="text/javascript">
        ...
        function update_todays_deal(el){
            ...
            $.post('{{ route('products.todays_deal') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    ...
                    AIZ.plugins.notify('danger', '{{ translate('You cannot set a product as Top deal if it has no discount') }}');
                    el.checked = false;
                }
                else{
        ...
        function update_new_arrival(el){
            ...
            $.post('{{ route('products.new-arrival') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('New Arrival updated successfully') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
