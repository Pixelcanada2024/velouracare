...
export default function PhoneInput({
  ...
  const defaultCountryCode = usePage().props.preferences?.country?.toLowerCase() || (isSkyAmerica ? 'us' : 'sa');
  ...
  // Dynamic Tailwind classes based on validation state
  const containerClasses = `
    ...
    [&_.react-international-phone-country-selector-button]:p-2
  ...
  return (
    <div className={`mb-4 ${className} ${containerClasses}`} >
      {label && (
      ...
      <div className="relative" dir="ltr">
        ...
        {isValid && (
          <div className="absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none" dir='rtl'>
      ...
      {hasError && (
        <div className="mt-1 text-sm text-red-500" dir='rtl'>
          {error}
        </div>
      )}
      ...
      {isValid && !hasError && (
        <div className="mt-1 text-sm text-green-600" dir='rtl'>
          {tr["phone_valid"]}
        </div>
      )}

      {/* Display validation error for invalid phone */}
