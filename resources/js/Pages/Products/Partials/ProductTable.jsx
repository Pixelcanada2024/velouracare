...
export default function ProductTable({ products, selectedItemsState, quantityControl }) {
  ...
  if (!products || products.length === 0) {
    return (
      <div className='container mx-auto max-sm:hidden my-20'>
  ...
  return (
    <div className='xl-container mx-auto max-sm:hidden max-lg:[&_td]:!text-[0.8em] max-lg:[&_td]:font-bold'>
      <div className="bg-white overflow-hidden">
        <div className="overflow-x-auto">
          <table className="min-w-full table-auto [&_th]:px-2 [&_td]:px-2">
            <thead className="bg-[#F2F4F7]  ">
              <tr>
                ...
                <th></th>
                <th className=" py-6 rtl:text-right lrt:text-left text-xs font-medium w-[150px] lg:w-[200px] xl:w-[250px]" >
                  {tr["product"]}
                </th>
                {/* <th className=" py-6 text-center text-xs font-medium ">
                  {tr['MOQ']} ({tr['pcs']})
                </th> */}
                <th className=" py-6 text-center text-xs font-medium ">
                  {tr["box_quantity"]}
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {products.data.map((product) => (
                <tr key={product.id} className="hover:bg-gray-50">
                    ...
                    <td>
                      {product?.qty > 0 && (
                        <input type="checkbox" name="checkedProducts[]"
                          ...
                          className="accent-primary-500 w-4 h-4 ml-4 product-checkbox cursor-pointer" />
                      )}
                    </td>
                  )}
                  <td className=" py-2">
                    <div className="h-16 w-16 lg:h-24 lg:w-24 mx-auto align-middle text-center">
                      <Link href={route("react.product", { product: product.id })}>
                        <img
                          className="h-16 w-16 lg:h-24 lg:w-24 rounded object-cover"
                  ...
                  <td className=" py-4">
                    <Link href={route("react.product", { product: product.id })}>
                      <div className="text-[10px] xl:text-xs font-medium text-[#9CA3AF] uppercase">
                        {product.brand}
                      </div>
                      <div className="text-[13px] xl:text-sm  font-semibold line-clamp-2" title={product.name}>
                        {product.name}
                      </div>
                      <div className="text-[10px] xl:text-xs text-[#9CA3AF] mt-1">
                        {tr["SKU"]}: {product.sku}
                        <br />
                        ...
                        {isAuthenticated && (
                          <>
                            <br />
                            <span className='text-[#565656]'>{tr["MOQ"]}</span><span className='text-black'> 1 {tr["box"]} ({product.box_qty}{tr["pcs"]})</span>
                          </>
                        )}
                      </div>
                    </Link>
                    ...
                    <td className=" py-4 ">
                      ...
                      <div className='flex flex-col gap-2 items-center'>
                        <div className="flex items-center space-x-2">
                          ...
                          <div className="text-sm font-medium rounded-lg px-5 lg:px-8 py-1 border-2 border-[#E1E4EA]">{cartQuantities?.[product.id] || 0}</div>
                        ...
                        <div className="text-[10px] xl:text-xs  mt-1">
                  ...
                  {isAuthenticated && (
                    ...
                    <td className="py-4 text-center ">
                      <div className="text-[10px] lg:text-xs xl:text-sm">
                  ...
                  {isAuthenticated && (
                    <td className="py-4">
                      <div className='flex justify-center items-center'>
                        {product?.qty >= (+(cartQuantities?.[product.id] || 0) * product?.box_qty) && product?.qty != 0 ? (
                          <Button
                            className='cursor-pointer rounded-sm w-15 xl:w-60'
                            onClick={() => addSelectedItemsToCart([product.id])}
                          >
                            <div className='flex items-center gap-2'>
                              <span className='text-xs xl:text-sm font-medium hidden xl:block'>{tr["add_to_cart"]}</span>
                              <WhiteCartIcon />
                            </div>
                          </Button>
                        ) : (
                          (() => {
                            ...
                            return (
                              <Button
                                ...
                                className={`flex items-center gap-2 rounded-sm w-15 xl:w-60 py-[8px] sm:py-[7px] xl:py-[11px] !text-sm ${isSubscribed ? "!border-[#CECECE]" : ""
                                  }`}
                              >
                                <span className="text-xs xl:text-sm font-medium hidden xl:block">
                  ...
                  {!isAuthenticated && (
                    <td colSpan={5} className='!p-0'>
                      <div className="flex justify-between items-center bg-[#F8F8F8] p-4 my-4">
                        ...
                        <Button
                          ...
                          className="cursor-pointer rounded-md uppercase !border-[#CECECE] !bg-white min-w-60"
                          onClick={() => addSelectedItemsToCart([product.id])}
                        >
                          {tr["login"]}
                        </Button>
