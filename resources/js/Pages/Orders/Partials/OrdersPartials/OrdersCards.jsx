...
export default function OrdersCards({ orders }) {
  const { queryParams, orderStatusOrder } = usePage().props;
  const [{ lang, currency, tr }, _setTranslation] = useTranslation();

  const OrderItem = " text-[13px] sm:text-base";
  const OrderItemValue = " text-[13px] sm:text-base";
  ...
  return (
    <>
      <div className='xl:hidden'>
        <div className="flex max-sm:flex-col sm:items-center justify-between max-sm:mb-6 gap-4">
          <p className='text-[34px] sm:text-[40px] xl:text-[52px] font-bold sm:my-4'>{tr["orders_list"]}</p>
          <SearchInput
        ...
        {!!orders?.data?.length ? orders?.data?.map((order) => (
          <div className="border-b border-gray-200" key={order.id}>
            <div className="bg-white">

              <div className='border-b border-[#E5E7EB] divide-y divide-[#E5E7EB]'>

                <div className='flex items-center justify-between px-3 py-2 bg-[#F2F4F7]'>
                  <p className={OrderItem}>{tr["order_id"]}</p>
                  <p className={OrderItemValue} dir="ltr">#{order.code}</p>
                </div>

                <div className='flex items-center justify-between px-3 py-2 '>
                  <p className={OrderItem}>{tr["products"]}</p>
                  <div className={`${OrderItemValue} sm:!text-[16px]`}>
                ...
                <div className='flex items-center justify-between px-3 py-2 '>
                  <p className={OrderItem}>{tr["total_quantity"]}</p>
                  <p className={OrderItemValue}>
                ...
                <div className='flex items-center justify-between px-3 py-2 '>
                  <p className={OrderItem}>{tr["total_amount"]}</p>
                  {orderStatusOrder[order.delivery_status] >= orderStatusOrder["confirmed"] 
                  ? <p className={OrderItemValue}><span dir='ltr'>{tr[order.additional_info["invoice"]["currency"]] ?? "SAR"}{(Number(order.grand_total) || 0).toFixed(2)}</span></p> 
                  : <p className={OrderItemValue}>-</p>}
                </div>

                <div className='flex items-center justify-between px-3 py-2 '>
                  <p className={OrderItem}>{tr["date"]}</p>
                  <p className={OrderItemValue}>{new Date(order.date * 1000).toISOString().split("T")[0]}</p>
                </div>

                <div className='flex items-center justify-between px-3 py-2 '>
                  <p className={OrderItem}>{tr["status"]}</p>
                  <p className={OrderItemValue}>
                    <span
                      className={`
                        text-[13px] sm:text-sm py-[2px] px-3 min-w-30 max-w-30 rounded-2xl capitalize my-1 inline-block text-center
                ...
                <div className='flex items-center justify-between px-3 py-2 '>
                  <p className={OrderItem}>{tr["pdf_invoice"]}</p>
                  <Link className='flex flex-col  text-center justify-center items-center' href={
                    orderStatusOrder[OrderItem.delivery_status] >= orderStatusOrder["confirmed"]
                      ? route("orders.print", OrderItem.id)
                      : "#"
                  }
                    title={
                      orderStatusOrder[OrderItem.delivery_status] >= orderStatusOrder["confirmed"]
              ...
              <div className='flex justify-center items-center py-4 text-xs sm:text-sm'>
                <Link
                  href={route("react.dashboard.orders.show", order.id)}
                  className="font-medium underline text-[#004AAD] "
                >
                  {tr["view_details"]}
                </Link>
              </div>
          ...
          <div className="my-2 border-gray-200">
            <div className="bg-[#F2F4F7] py-22 flex flex-col items-center justify-center">
              <div className="mb-4">
